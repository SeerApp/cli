...
pub fn build_all_programs(programs: &[crate::project::SolanaProgram], seer_toml_paths: &[(String, PathBuf)]) -> Vec<BuildResult> {
...
pub fn build_all_programs_silent(programs: &[crate::project::SolanaProgram], seer_toml_paths: &[(String, PathBuf)]) -> Vec<BuildResult> {
    let mut build_results = Vec::new();
    for (i, prog) in programs.iter().enumerate() {
        println!("Building {}...", prog.name);
        let seer_toml_path = &seer_toml_paths[i].1;
